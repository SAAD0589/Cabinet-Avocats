    ...
    <div class="container my-5" id="FormOne">
        ...
        <div class="row d-flex justify-content-end my-5" >
            <form action="{{ route('RecuperationUpdate1',$recuperationcheck->id) }}" method="post" enctype="multipart/form-data" >
                <div class="">
                    @csrf
                    <div class="row py-2 justify-content-center">
                        <div class="{{app()->getLocale() === 'ar' ? 'col-5': 'col-xl-5 col-sm-6'}}">
                            ...
                            <div class="row">
                                <div class="col-xl-4 p-2 col-sm-7">
                                    <input type="radio" name="type_clt" id="input1" value="clt1"
                                        {{ $Usercheck->type_clt == 'clt1' ? 'checked' : '' }}>
                                    <label class="fw-bold">{{ __('content.شخص معنوي') }}</label>
                                </div>
                                <div class="col-xl-4 p-2 col-sm-5">
                                    <input type="radio" name="type_clt" id="input2" value="clt2"
                                        {{ $Usercheck->type_clt != 'clt1' ? 'checked' : '' }}>
                                    <label class="fw-bold">{{ __('content.شخص ذاتي') }}</label>
                                </div>
                            </div>
                        </div>
                        <div class="col-5">
                            ...
                            <div class="row">
                                <div class="{{app()->getLocale() === 'ar' ? 'col-xl-4 col-sm-6': 'col-xl-5 col-sm-7'}} p-2">
                                    <input type="radio" name="id_type_dossier"
                                        {{ $recuperationcheck->id_type_dossier == '5' ? 'checked' : '' }} value="5">
                                    <label class="fw-bold">{{ __('content.استرجاع السيارة') }}</label>
                                </div>
                                <div class="{{app()->getLocale() === 'ar' ? 'col-xl-4 col-sm-6': 'col-xl-5 col-sm-5'}} p-2">
                                    <input type="radio" name="id_type_dossier"
                                        {{ $recuperationcheck->id_type_dossier != '5' ? 'checked' : '' }} value="6">
                                    <label class="fw-bold">{{ __('content.قرض شخصي') }}</label>
                                </div>
                            </div>
                        ...
                        <div class="col-xl-5 col-sm-6 py-2">
                            <select class="inputcheck form-select" name="id_clt" id="id_clt"
                                style="font-weight: bold;font-size:18px" aria-label="Default select example">
                                <option value="" disabled>{{ __('content.الموكل') }}</option>
                                @foreach ($users as $user)
                                    <option @if ($Usercheck->id == $user->id) selected @endif value="{{ $user->id }}">
                                        {{ $user->name }}</option>
                                @endforeach
                                {{-- <option selected value="{{$Usercheck->id}}" >{{$Usercheck->name}}</option> --}}
                        ...
                        <div class="col-xl-5 col-sm-6 py-2">
                            <div class="relative">
                                <input class="inputcheck input-cal input-base  form-control" id="input" placeholder=""
                                 type="text" name='reference' readonly value="{{ $recuperationcheck->reference }}"
                            ...
                            {{-- <select class="inputcheck form-select" name="dossier_id" style="font-weight: bold;font-size:18px"
                                aria-label="Default select example">
                                <option value="" disabled>{{ __('content.المرجع') }}</option>
                                <option selected value="{{ $dossiercheck->reference }}">{{ $dossiercheck->reference }}
                                </option>
                            </select> --}}
                        ...
                        <div class="col-xl-5 col-sm-6 py-2">
                            <div class="relative">
                                <input class="inputcheck input-cal input-base  form-control" id="input" placeholder=""
                                value="{{ $recuperationcheck->enmy }}" name="enmy" type="text">
                                <label id="label-input" class="fw-bold" style="{{app()->getLocale() === 'ar'? 'right: 10px' :'left: 10px' }}">{{ __('content.الخصم') }}</label>
                            </div>
                            @error('enmy')
                            <p class=" text-danger">{{ $message }}</p>
                            @enderror
                            {{-- <div class="group">
                                <input type="text" class="form-control" placeholder="الخصم" name="enmy"
                                    value="{{ $recuperationcheck->enmy }}">
                                <label dir="rtl" class="TextInput fw-bold mx-2"></label>
                        ...
                        <div class="col-xl-5 col-sm-6 py-2">
                            <input type="date" id="form12" class="inputcheck form-control" name="date_Rec"
                                value="{{ $recuperationcheck->date_Rec }}" />
                            @error('date_Rec')
                                <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
