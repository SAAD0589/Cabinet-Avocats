...
@section('content')
    ...
    <div class="container ">
        <div class="d-flex justify-content-between my-1 align-items-center" style="height: 120px">
            <div class=" w-100 ">
                <h3 class="fw-bold text-center px-5" style="margin-right:90px "> تعديل طلب تبليغي</h3>
            </div>
            <div class="">
                    ...
                    <div class="row py-2 justify-content-center">
                        <div class="col-xl-5 col-sm-6 py-2">
                            <input type="date" class="form-control" dir="rtl" placeholder="تاريخ وضع الطلب"
                                name="date_R" style="font-weight: bold;font-size:18px" value="{{ $report->date_R }}">
                            @error('date_R')
                                <p class="text-end text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="col-xl-5 col-sm-6 py-2">
                            <input type="text" class="form-control " dir="rtl" placeholder="رقم الملف التبليغي "
                                name="Num_Doc_R" style="font-weight: bold;font-size:18px" value="{{ $report->Num_Doc_R }}">
                            @error('Num_Doc_R')
                                <p class="text-end text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="col-xl-5 col-sm-6">
                            <div class="row ">
                                <h5 class="fw-bold text-end">نوع الموكل</h5>

                                 <div class="col-xl-4 col-sm-6 text-end py-2 ">
                        ...
                        <div class="col-xl-5 col-sm-6">
                            ...
                            <select class="form-select" aria-label="Default select example" id="id_clt"
                                ...
                                @foreach ($reportChecks as $reportCheck)
                                    <option value="{{ $reportCheck->id }}">{{ $reportCheck->reference }} /
                                        {{ $reportCheck->type_nom }} </option>
                                @endforeach

                            </select>
                        </div>
                        <div class="col-xl-5 col-sm-6 py-2">
                            <input type="text" class="form-control " dir="rtl" placeholder="اسم المفوض القضائي"
                                name="judicial_commisioner" style="font-weight: bold;font-size:18px"
                                value="{{ $report->judicial_commisioner }}">
                            @error('judicial_commisioner')
                                <p class="text-end text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="col-xl-5 col-sm-6 py-2">
                            <input type="text" class="form-control " dir="rtl" placeholder="الإجراء المطلوب "
                                name="procedure" style="font-weight: bold;font-size:18px" value="{{ $report->procedure }}">
                            @error('procedure')
                                <p class="text-end text-danger">{{ $message }}</p>
                            @enderror
                        </div>
