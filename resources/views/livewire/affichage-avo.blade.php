...
<div class="row py-5">
    <div class="table-responsive">
        @include('Admins.avocats.Delete')
        <table class="table ">
            <thead style="border-bottom:1pt solid #fff">
                <tr class="">
                    <th scope="col" class="text-truncate">{{ __('content.الإسم و النسب') }}</th>
                    <th scope="col" class="text-truncate">{{ __('content.تخصص') }}</th>
                    <th scope="col" class="text-truncate"> 
                        {{ __('content.البريد الإلكتروني') }}
                    </th>
                    <th scope="col" class="text-truncate">{{ __('content.الهاتف') }}</th>
                ...
                <tr class="">
                    ...
                    <th scope="col">
                        <div style="position: relative;">
                            <div style="position: relative; ">
                                <input type="text" class="inputAff form-control" wire:keydown='searchName' name="Specialise" wire:model="Specialise" value="">
                                <i class="bi bi-search mx-2" style="position: absolute; {{ app()->getLocale() === 'ar' ? 'left' : 'right' }}: 4px; top: 50%; transform: translateY(-50%); color: #C09F5E;"></i>
                            </div>
                    ...
                    <th scope="col">
                      ...
                      <th scope="col" class="">
                        <select class="inputAff form-select " aria-label="Default select example" 
                            ...
                            <option selected value="" class="">{{ __('content.الحالة') }}</option>
                ...
                @foreach ($data as $item)
                    <tr class="my-5 " style="height: 60px">
                         <td class="fw-bold text-truncate">{{ $item->name }} {{ $item->LastName }}</td>
                         <td class="fw-bold text-truncate">{{ $item->Specialise }}</td>
                         <td class="fw-bold text-truncate">{{ $item->email }}</td>
                         <td class="fw-bold text-truncate">{{ $item->tel != null ? $item->tel : 'لا يوجد' }}</td>
                         <td class="">
                            @if ($item->status == 'active')
                                <img class="mx-5" src="{{ asset('assets/logo/check.png') }}" style="height: 25px"/>
                            @else
                                <img class="mx-5" src="{{ asset('assets/logo/close.svg') }}" style="height: 20px"/>
                            @endif
                        </td>
                        <th scope="row">
                            <div class="dropdown ">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <li><a class="dropdown-item text-center" style="font-weight: bold;font-size:18px"
                                            href="{{ route('avocats.edit', $item->id) }}">{{ __('content.تعديل') }}</a></li>
                                    <li><button style="font-weight: bold;font-size:18px "
                                            class="DeleteAvo dropdown-item text-center text-danger text-capitalize" data-toggle="modal"
                                            data-toggle="modal" data-target="#DeletedAvo"
...
<script>
    ...
    $(document).ready(function() {
        $(document).on('click', '.DeleteAvo', function() {
            var idAvo = $(this).attr('data-id');
            $('#FromAvo').val(idAvo);
